...
项目根目录的 `审稿修订执行记录.md` 是审稿修订的长期执行记录和跨对话交接依据。
每次修订过程中必须实时更新该记录；开始一项工作时登记目标、影响范围和前置分析，
完成一项工作后登记当前进度、本次改动、具体实现方式、采用的方法或措施、验证命令
及其结果、仍存风险和下一步。记录应同时保存所用配置、场景、随机种子、模型与
`VecNormalize` 配对路径以及结构化结果路径（如适用）。在不同对话或由不同 Agent
接手时，必须先阅读该记录并在继续工作前更新交接状态；不得只依赖口头总结或 Git
提交信息替代该记录。
